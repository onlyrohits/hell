import {
  ...
  RapidaLogger,
  ResponseBody,
} from "./../async_logger/RapidaLogger";
...
describe("Rapida Proxy OpenAI tests", () => {
  ...
  test("CUSTOM_MODEL", async () => {
    ...
    const options: IRapidaAsyncClientOptions = {
      rapidaMeta: {
        apiKey: TEST_HELICONE_API_KEY,
        baseUrl: TEST_ASYNC_URL,
      },
    ...
    const logger = new RapidaLogger(options);

    const llmArgs = {
      model: "llama-2",
      prompt: "Say hi!",
    };
    const builder = new RapidaLogBuilder(llmArgs);
