...
from rapida.openai_proxy import openai, log_feedback
...
rapida_global.proxy_url = "http://127.0.0.1:8787/v1"
rapida_global.api_key = os.getenv("HELICONE_API_KEY_LOCAL")
...
def test_cache_completion():
    ...
    assert response1.rapida.cache == "MISS"
    ...
    assert response2.rapida.cache == "HIT"

    response1_copy = response1.copy()
    response2_copy = response2.copy()

    del response1_copy['rapida']['cache']
    del response2_copy['rapida']['cache']
...
def test_rate_limit_policy():
    ...
    assert response.rapida.rate_limit.policy.startswith(
...
def fetch_feedback(rapida_id):
    api_key_hash = hash(f'Bearer {os.getenv("HELICONE_API_KEY_LOCAL")}')

    # Fetch the response with the corresponding rapida_id
    response_query = sb.table("response").select(
        "id, request").eq("request", rapida_id)
    ...
    request_query = sb.table("request").select(
        "id, rapida_api_keys (id, api_key_hash)").eq("id", response_data["request"])
    request_result = request_query.single().execute()
    request_data = request_result.data

    matching_api_key_hash = request_data["rapida_api_keys"]["api_key_hash"]
    matching_api_key_id = request_data["rapida_api_keys"]["id"]

    # Check if the apiKeyHash matches the rapida_api_key_id's api_key_hash
    ...
    metric_query = sb.table("feedback_metrics").select("id, name, data_type").eq(
        "rapida_api_key_id", str(matching_api_key_id))
...
def test_log_feedback():
    ...
    rapida_id = response['rapida']['id']
    feedback_data = fetch_feedback(rapida_id)
...
def test_sync_stream():
    ...
    rapida_id = last_chunk['rapida']['id']
    feedback_data = fetch_feedback(rapida_id)
